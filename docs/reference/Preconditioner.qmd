...
| [grad_Q](#deep_tensor.Preconditioner.grad_Q) | Evaluates the mapping $Q(\cdot)$ and its gradient. |
...
### grad_Q { #deep_tensor.Preconditioner.grad_Q }

```python
Preconditioner.grad_Q(us: Tensor, subset: str = 'first')
```

Evaluates the mapping $Q(\cdot)$ and its gradient.
...
<code>[**dxdus**]{.parameter-name} [:]{.parameter-annotation-sep} [Tensor]{.parameter-annotation}</code>

:   A $k \times n \times k$ tensor, where `dxdus[:, i, :]`  contains the Jacobian of $Q(\cdot)$ evaluated at `xs[i, :]`.