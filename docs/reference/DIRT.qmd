...
| [eval_pdf](#deep_tensor.DIRT.eval_pdf) | Evaluates the density function. |
...
### eval_pdf { #deep_tensor.DIRT.eval_pdf }
...
:   The number of layers of the current DIRT construction to  use. If not specified, all

#### Returns {.doc-section .doc-section-returns}

<code>[**fxs**]{.parameter-name} [:]{.parameter-annotation-sep} [[Tensor](`torch.Tensor`)]{.parameter-annotation}</code>

:   An $n$-dimensional vector containing the value of the  approximation to the target density evaluated at each  element in `xs`.
...
:   An $n$-dimensional vector containing the potential function  of the approximation to the conditional density of  $X \textbar Y$ evaluated at each sample in `rs`.
