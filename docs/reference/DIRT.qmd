...
:   A $1 \times k$ (if the same realisation of $Y$ is to be  used for all samples in `rs`) or $n \times k$ matrix (if a  different realisation of $Y$ is to be used for all samples  in `rs`) containing samples from the approximation domain.
