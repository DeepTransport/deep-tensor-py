...
class HeatSolver(object):
    ...
    def vec2func(self, vec: dl.Vector | Tensor, component: int) -> dl.Function:
...
class HeatSolverROM(object):
    ...
    def __init__(self, solver: HeatSolver, m: Tensor, V: Tensor):
    ...
    def reduced_to_full(self, u: Tensor) -> Tensor:
        return self.V @ u # + self.m

    def solveFwd(self, k: Tensor) -> None:
        ...
        # Convert prior coefficients to corresponding function
        k = self.prior.transform(k)

        # Assemble stiffness matrix (TODO: fix this...)
        K = ufl.exp(k) * ufl.inner(ufl.grad(self.u), ufl.grad(self.v)) * ufl.dx
        K = dl.assemble(K)
        K = torch.tensor(K.array())

        K_r = self.V.T @ K @ self.V
        ...
        us = torch.zeros((self.r, self.nt))

        for i in range(self.nt):

            b_r = self.M_r @ (self.dt * self.f_r + u_r_prev)

            u_r = torch.linalg.lu_solve(*lu, b_r[:, None]).flatten()

            us[:, i] = u_r
            u_r_prev = u_r.clone()
        
        return self.reduced_to_full(us)