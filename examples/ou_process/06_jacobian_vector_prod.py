...
for poly in polys_dict:
    for method in methods:
        for direction in directions:
            ...
            t0 = time.time()
            Jvs = sirt.eval_rt_jac_prod(xs, vs)
            t1 = time.time()

            Js_fd = compute_finite_difference_jac(sirt, xs)
            Jvs_fd = Js_fd @ vs.flatten()
            Jvs_fd = Jvs_fd.reshape(n_zs, dim)

            plt.figure(figsize=(6, 6))
            plt.scatter(Jvs.flatten(), Jvs_fd.flatten(), s=4)
            plt.title("Jacobian Finite Difference Check")
            plt.ylabel(r"$J_{\mathrm{fd}}$")
            plt.xlabel(r"$J$")
            plt.savefig(f"examples/ou_process/figures/06_jvp_{poly}_{method}_{direction}.pdf")
            plt.clf()

            approx_error = norm(Jvs - Js_fd)
